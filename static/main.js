...
window.onload = async () => {
    const station = {
        1: { station_name: "Bengaluru", buses_scheduled: [300, 1200], buses_available: [100, 1000, 200, 300, 1200] },
        2: { station_name: "Mysuru", buses_scheduled: [300], buses_available: [100, 1000, 500, 300, 800] },
        3: { station_name: "Puducherry", buses_scheduled: [1200], buses_available: [1200] }
    }
    const bus_details = {
        100: { source: "Benagluru", destination: "Mysuru" },
        200: { source: "Bellari", destination: "Benagluru" },
        1200: { source: "Benagluru", destination: "Puducherry" },
        300: { source: "Benagluru", destination: "Mysuru" },
        500: { source: "Bellari", destination: "Mysuru" },
        800: { source: "Mysuru", destination: "Mangaluru" }
    }
    console.log('The Script will load now. check bus availability, view bookings');
    let msg = ""
    msg += `For answering questions, only refer to this data, not anything else. The station object of the database is like ${station}, so any question asked should be checked with this object.`
    msg += `Station 1 = ${station[1].station_name}, Buses Scheduled = ${station[1].buses_scheduled}, Buses Available = ${station[1].buses_available}. If a bus is not yet scheduled, you should say so. If the bus is not present at all, say so.`
    msg += `Station 2 = ${station[2].station_name}, Buses Scheduled = ${station[2].buses_scheduled}, Buses Available = ${station[2].buses_available}. If a bus is not yet scheduled, you should say so. If the bus is not present at all, say so.`
    msg += `Station 3 = ${station[3].station_name}, Buses Scheduled = ${station[3].buses_scheduled}, Buses Available = ${station[3].buses_available}. If a bus is not yet scheduled, you should say so. If the bus is not present at all, say so.`
    for (let key in bus_details) {
        msg += `Bus Number : ${key} Bus source: ${bus_details[key].source} Bus Destination : ${bus_details[key].destination}`
    }

    msg += "This chatbot is part of a bus booking system, so you will answer only those questions related to bus stuff, anything else state - This chatbot can answer questions only about bus booking.... only till here not next line"
    ...
    // msg += "Database status as of now: for Bengaluru station Bus 100 travels between Bengaluru and Mysuru"
